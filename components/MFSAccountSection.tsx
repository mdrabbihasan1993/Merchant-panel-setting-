...
export const MFSAccountSection: React.FC = () => {
  ...
  const getValidationError = (id: string, val: string): string => {
    ...
    // Only validate prefix if at least 3 digits are typed
    if (val.length >= 3) {
    ...
    // Don't show length error while typing unless it exceeds (which handleInputChange prevents anyway)
    // We only show the length error if they type exactly the max and it's wrong, or on Save.
    return ''; 
  };
  ...
  const handleInputChange = (id: string, val: string) => {
    ...
    // 1. Clean input: only digits
    const cleanVal = val.replace(/\D/g, '');
    
    // 2. Limit length
    if (cleanVal.length > maxLen) return;

    // 3. Update state regardless of "error" to ensure smooth typing
  ...
  const handleSave = () => {
    ...
    Object.keys(newAccounts).forEach(id => {
      ...
      if (!acc.value) return; // Skip empty fields
    ...
    if (!globalError) {
      const hasAny = Object.values(accounts).some(a => a.value !== '');
      if (hasAny) {
        ...
        <div className="pt-6 border-t border-slate-50 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-1">
            ...
            <p className="text-xs text-slate-400 font-medium leading-relaxed max-w-sm">
              • Numbers must start with: 013, 014, 015, 016, 017, 018, or 019.<br/>
